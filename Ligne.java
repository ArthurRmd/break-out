...
class Ligne {
    ...
    public void decalageLigneVersDroite () {


    boolean caseVide = false;
    int nombreDecalge = 0;

    while ( !caseVide && nombreDecalge < tableau.length) {

        if ( tableau[nombreDecalge].getContenu() == 0) {
            caseVide = true;
        }
        nombreDecalge++;
    }
    ...
    nombreDecalge--;
        ...
        if (caseVide == false ) {
            ...
            nombreDecalge = tableau.length -1 ;

        }



       for ( int i = nombreDecalge  ; i > 0 ; i-- ) {

           tableau[nombreDecalge ].setContenu( tableau[nombreDecalge -1].getContenu() );
          // System.out.println("OK" + tableau[taille-1].getContenu());

           nombreDecalge--;
       }

       tableau[0].setContenu(0);

    }



    public  void decalageLigneVersGauche () {



        boolean caseVide = false;
        int nombreDecalge = tableau.length - 1;

        while ( !caseVide && nombreDecalge > 0) {
        ...
        for ( int i = nombreDecalge  ; i < tableau.length -1; i++ ) {
